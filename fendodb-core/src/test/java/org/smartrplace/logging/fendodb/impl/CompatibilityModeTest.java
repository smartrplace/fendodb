...
public class CompatibilityModeTest extends SlotsDbTest {
	...
	private static void addingValuesWithRestartWorks(final List<SampledValue> values, final StorageType storageType, final boolean flushImmediately) throws DataRecorderException, IOException {
		...
		try (final SlotsDb instance2 = new SlotsDb(Paths.get(SlotsDb.DB_TEST_ROOT_FOLDER), null, null, null)) {
			Assert.assertTrue("Database not started in compatibility mode",instance2.getConfiguration().useCompatibilityMode());
	...
	private static void iteratorWorksInCompatMode(final List<SampledValue> values) throws IOException, DataRecorderException {
		final FendoDbConfiguration config = FendoDbConfigurationBuilder.getInstance()
			.setFlushPeriod(0)
			.setUseCompatibilityMode(true)
			.build();
	...
	public void iteratorWorks0() throws IOException, DataRecorderException {
		iteratorWorksInCompatMode(Arrays.asList(
	...
	@Test
	public void compatModeIsRecognizedInSpiteOfWrongConfig() throws IOException, DataRecorderException {
		deleteTestFiles();
		final FendoDbConfiguration config = FendoDbConfigurationBuilder.getInstance()
			.setFlushPeriod(0)
			.setUseCompatibilityMode(true)
			.build();
		final String path;
		final List<SampledValue> values = DATASET1;
		try (final SlotsDb instance = new SlotsDb(Paths.get(SlotsDb.DB_TEST_ROOT_FOLDER), null, config, null)) {
			final RecordedDataConfiguration cfg = new RecordedDataConfiguration();
			cfg.setStorageType(StorageType.ON_VALUE_UPDATE);
			final RecordedDataStorage data = instance.createRecordedDataStorage("test", cfg);
		...
		final FendoDbConfiguration config2 = FendoDbConfigurationBuilder.getInstance()
				.setFlushPeriod(0)
				.setUseCompatibilityMode(false) // not possible, since there is already data available in the old format
				.build();
		try (final SlotsDb instance2 = new SlotsDb(Paths.get(SlotsDb.DB_TEST_ROOT_FOLDER), null, config2, null)) {
			Assert.assertTrue("Database not started in compatibility mode",instance2.getConfiguration().useCompatibilityMode());
		}
		deleteTestFiles();
	}
